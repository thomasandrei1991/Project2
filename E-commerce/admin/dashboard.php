<?php
    ...
    $r = mysqli_query($conn, "SELECT COALESCE(SUM(total),0) AS c FROM orders WHERE status != 'cancelled'");
    $stats['revenue'] = mysqli_fetch_assoc($r)['c'];

    // Recent orders
    $recentOrders = mysqli_fetch_all(mysqli_query($conn, "SELECT o.*, u.fullname FROM orders o JOIN users u ON o.user_id = u.id ORDER BY o.created_at DESC LIMIT 5"), MYSQLI_ASSOC);

    // Low stock products
    $lowStock = mysqli_fetch_all(mysqli_query($conn, "SELECT * FROM products WHERE stocks <= 5 ORDER BY stocks ASC LIMIT 5"), MYSQLI_ASSOC);
?>
<!DOCTYPE html>
<html lang="en">
    <head>
                ...
                <div class="dashboard-grid">
                    ...
                    <div class="admin-card">
                        <div class="card-header">
                            ...
                            <a href="<?php echo BASE_URL; ?>admin/products.php" class="view-all">Manage →</a>
                        </div>
                                ...
                                <tbody>
                                    <?php foreach ($lowStock as $p): ?>
                                        <tr>
                                            ...
                                            <td><span class="badge badge-<?php echo $p['stocks'] == 0 ? 'cancelled' : 'pending'; ?>"><?php echo $p['stocks']; ?> left</span></td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
